...
async function generateDemo(data) {
        ...
        if (stats.isDirectory()) {
            ...
            if (textExtensions.includes(ext)) {
                ...
                if (ext === '.html') {
                    ...
                    const floatingCTA = `
    <!-- Teadustech SiteSpark Shared Template Upgrade -->
    <style>
    ...
    <!-- Teadustech SiteSpark Demo Mode Footer -->
    <style>
...
async function deployToGithub(targetDir, repoName, token, orgName) {
    ...
    const createRes = await fetch(`https://api.github.com/orgs/${orgName}/repos`, {
        ...
        body: JSON.stringify({
            name: repoName,
            description: 'Automated demo website generated by Teadustech SiteSpark',
            private: false,
            auto_init: false
        })
    });
    ...
    for (const filePath of filesToUpload) {
        ...
        const uploadRes = await fetch(`https://api.github.com/repos/${orgName}/${repoName}/contents/${relativePath}`, {
            ...
            headers: githubHeaders,
            body: JSON.stringify({
                message: `Add ${relativePath} via Teadustech SiteSpark`,
